...
class CarpoolRequestUpdatedNotification extends Notification
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        if($notifiable->hasRole('carpool_driver')){
            return (new MailMessage)
                ->subject(__('Carpool Request Updated'))
                ->greeting(__('Hello, ' . $notifiable->name . '!'))
                ->line(__('A carpool request has been updated.'))
        ...
        return (new MailMessage)
            ->subject(__('Carpool Request Updated'))
            ->greeting(__('Hello, ' . $notifiable->name . '!'))
            ->line(__('Your carpool request has been updated.'))
            ->line(__('Title: ' . $this->carpoolRequest->title))
            ->line(__('Description: ' . $this->carpoolRequest->description))
            ->line(__('Schedule Date: ' . $this->carpoolRequest->departure_date))
            ->line(__('Schedule Time: ' . $this->carpoolRequest->departure_time))
    ...
    public function toArray(object $notifiable): array
    {
        if($notifiable->hasRole('carpool_driver')){
            return [
                'subject' => __('Carpool Request Updated'),
                'message' => __('A carpool request has been updated.'),
        ...
        return [
            'subject' => __('Carpool Request Updated'),
            'message' => __('Your carpool request has been updated.'),
