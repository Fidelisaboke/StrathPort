<x-admin-app-layout>
    ...
    <div class="container grid w-3/5 px-6 mx-auto">
        <div class="items-center p-4 my-6">
            <form method="post" action="{{ route('admin.transport_requests.update', $transportRequest->id) }}">
                @csrf
                @method('PUT')
                <div class="overflow-hidden shadow sm:rounded-md">
                    ...
                    <div class="px-4 py-5 bg-white sm:p-6">
                        ...
                        <select name="status" id="status" class="block w-full mt-1 rounded-md shadow-sm form-select">
                            <option value="Pending" {{ old('status', $transportRequest->status) == 'Pending' ? 'selected' : '' }}>Pending</option>
                            <option value="Approved" {{ old('status', $transportRequest->status) == 'Approved' ? 'selected' : '' }}>Approved</option>
                            <option value="Declined" {{ old('status', $transportRequest->status) == 'Declined' ? 'selected' : '' }}>Declined</option>
                        </select>
                        @error('status')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                        @enderror
