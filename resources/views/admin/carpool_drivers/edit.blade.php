<x-admin-app-layout>
    ...
    <div class="container grid w-3/5 px-6 mx-auto">
        <div class="items-center p-4 my-6">
            <form method="post" action="{{ route('admin.carpool_drivers.update', $carpoolDriver) }}">
                @csrf
                @method('PUT')
                <div class="overflow-hidden shadow sm:rounded-md">
                    ...
                    <div class="px-4 py-5 bg-white sm:p-6">
                        ...
                        <input type="text" name="first_name" id="first_name" type="text"
                            ...
                            value="{{ old('first_name', $carpoolDriver->first_name) }}" />
                        @error('first_name')
                    ...
                    <div class="px-4 py-5 bg-white sm:p-6">
                        ...
                        <input type="text" name="last_name" id="last_name" type="text"
                            class="block w-full mt-1 rounded-md shadow-sm form-input"
                            value="{{ old('last_name', $carpoolDriver->last_name) }}" />
                        @error('last_name')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="px-4 py-5 bg-white sm:p-6">
                        ...
                        <select name="availability_status" id="availability_status"
                            ...
                            <option value="Available"
                                {{ old('availability_status', $carpoolDriver->availability_status) == 'Available' ? 'selected' : '' }}>
                                Available</option>
                            <option value="Unavailable"
                                {{ old('availability_status', $carpoolDriver->availability_status) == 'Unavailable' ? 'selected' : '' }}>
                                Unavailable</option>
                        </select>
