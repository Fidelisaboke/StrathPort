                            ...
                            <div class="flex flex-col">
                                ...
                                @if ($carpoolingDetail->carpoolRequest && $carpoolingDetail->status == 'In Progress')
                                    <div class="flex justify-center mt-4">
                                        <form
                                            action="{{ route('driver.carpooling_details.completeTrip', $carpoolingDetail->id) }}"
                                            method="POST" x-data>
                                            @csrf
                                            @method('PUT')
                                            <button
                                                @click.prevent="$dispatch('completeTrip', { redirectUrl: 'driver/carpooling_details', id: {{ $carpoolingDetail->id }}, modelClass: 'App\\Models\\CarpoolingDetails' })"
                                                type="submit"
                                                class="px-4 py-2 text-white bg-green-500 rounded hover:bg-green-600">Mark
                                                as Completed</button>
                                        </form>
                                        <form
                                            action="{{ route('driver.carpooling_details.cancelTrip', $carpoolingDetail->id) }}"
                                            method="POST" x-data>
                                            @csrf
                                            @method('PUT')
                                            <button
                                                @click.prevent="$dispatch('cancelTrip', { redirectUrl: 'driver/carpooling_details', id: {{ $carpoolingDetail->id }}, modelClass: 'App\\Models\\CarpoolingDetails' })"
                                                type="submit"
