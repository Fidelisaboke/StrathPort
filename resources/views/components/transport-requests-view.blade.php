        ...
        <div class="px-4 py-5 sm:px-6">
            ...
            <div class="mt-2 space-y-2 overflow-auto">
                ...
                <div class="flex flex-row flex-wrap">
                    ...
                    <p class="text-gray-800 text-md">Time: {{ \Carbon\Carbon::parse($transportRequest->event_time)->format('H:i')}}</p>
                </div>
                <div class="flex flex-row">
                    ...
                    <p class="text-gray-800 text-md">Location: {{$transportRequest->event_location}}</p>
                </div>
                <div class="flex flex-row">
                    <svg class="w-6 h-6 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
            ...
            <div class="flex flex-wrap items-center justify-between mt-4">
                ...
                <div class="flex flex-col justify-end space-y-2 md:flex-row md:space-x-2 md:space-y-0">
                    ...
                    <form class="inline-block" action="{{ route('transport_requests.destroy', $transportRequest->id) }}" method="POST" x-data>
                        @csrf
                        @method('DELETE')
                        <button @click.prevent="$dispatch('delete', { indexRoute: 'transport_requests.index', id: {{ $transportRequest->id }}, modelClass: 'App\\Models\\TransportRequest'});" type="submit" class="px-4 py-2 font-medium text-red-600 rounded hover:text-red-500 hover:bg-gray-100">Delete</button>
                    </form>
                </div>
            </div>
        </div>
