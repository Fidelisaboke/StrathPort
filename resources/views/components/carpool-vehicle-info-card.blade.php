    ...
    <figure class="relative flex flex-col-reverse p-6 rounded-lg">
        <blockquote class="mt-2 text-slate-700">
            ...
            <div class="justify-end mt-4 md:items-center md:flex-row md:flex">
                ...
                <a href="{{route('driver.carpool_vehicles.edit', $carpoolVehicle->id)}}" class="inline-flex items-center justify-center px-4 py-2 mr-3 font-medium text-center text-white transition ease-in-out delay-150 rounded-lg text-md hover:-translate-y-1 hover:scale-110 bg-fuchsia-700 hover:bg-fuchsia-800 focus:ring-4 focus:ring-pink-300 dark:focus:ring-fuchsia-900">
                ...
                <form method="post" action="{{ route('driver.carpool_vehicles.destroy', $carpoolVehicle->id) }}" x-data>
                    @csrf
                    @method('DELETE')
                    <button @click.prevent="$dispatch('delete', { indexRoute: 'driver.carpool_vehicles.index', id: {{ $carpoolVehicle->id }}, modelClass: 'App\\Models\\CarpoolVehicle'});" type="submit" class="items-center justify-center px-4 py-2 mt-4 font-medium text-center text-white transition ease-in-out delay-150 bg-red-700 rounded-lg md:mt-0 minline-flex text-md hover:-translate-y-1 hover:scale-110 hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
                        Delete Vehicle
                    </button>
                </form>
            </div>
