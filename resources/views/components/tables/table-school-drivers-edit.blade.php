@empty($schoolDrivers)
    <div class="flex items-center justify-center">
        <div class="w-1/2 p-4 mx-auto my-16 text-center bg-white border border-gray-300 rounded-lg shadow-md">
            <h2 class="text-2xl font-semibold text-gray-700">No school drivers found!</h2>
            <p class="mt-2 text-gray-500">Please add a new school driver.</p>
        </div>
    </div>
@else
                ...
                <tbody>
                    @foreach ($schoolDrivers as $schoolDriver)
                        <tr>
                            ...
                            <td class="p-4 border-b border-blue-gray-50">
                                <div class="flex items-center gap-3">
                                    @if ($schoolDriver->availability_status == 'Available')
                                        <div class="relative grid items-center px-2 py-1 font-sans text-xs font-bold text-green-600 uppercase rounded-md select-none whitespace-nowrap bg-green-500/20"
                                            ...
                                            <p class="block antialiased leading-none text-center">
                                                {{ $schoolDriver->availability_status}}</p>
                                        </div>
                                    @elseif($schoolDriver->availability_status == 'Unavailable')
                            ...
                            <td class="p-4 border-b border-blue-gray-50">
                                <!-- view -->
                                <a href="{{ route('admin.school_drivers.show', $schoolDriver->id) }}"
                                    class="text-green-600 rounded hover:text-green-800">View</a>
                                <!-- edit -->
                                <a href="{{ route('admin.school_drivers.edit', $schoolDriver->id) }}"
                                    class="ml-2 text-indigo-600 rounded hover:text-indigo-800">Edit</a>
                                <!-- delete -->
                                <form class="inline-block"
                                    action="{{ route('admin.school_drivers.destroy', $schoolDriver->id) }}"
                                    method="POST" x-data>
                                    @csrf
                                    @method('DELETE')
                                    <button
                                        @click.prevent="$dispatch('delete', { indexRoute: 'admin.school_drivers.index', id: {{ $schoolDriver->id }}, modelClass: 'App\\Models\\SchoolDriver', adminModule:true });"
                                        type="submit"
