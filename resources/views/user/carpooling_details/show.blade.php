...
<x-app-layout>
    ...
    <div class="py-4">
        <div class="max-w-6xl py-10 mx-auto sm:px-6 lg:px-8">
            <!-- Back Button -->
            <x-button-link href="{{ route('carpooling_details.index') }}" text="Back to Carpool Schedules List"
                arrowType="left" />
                            ...
                            <div class="flex justify-between">
                                ...
                                @if ($carpoolingDetail->carpoolRequest && $carpoolingDetail->status == 'In Progress')
                                    <div class="flex items-center">
                                        <form
                                            action="{{ route('carpooling_details.completeTrip', $carpoolingDetail->id) }}"
                                            method="POST" x-data>
                                            @csrf
                                            @method('PUT')
                                            <button
                                                @click.prevent="$dispatch('completeTrip', { redirectUrl: 'carpooling_details', id: {{ $carpoolingDetail->id }}, modelClass: 'App\\Models\\CarpoolingDetails' })"
                                                type="submit"
                                                class="px-4 py-2 text-white bg-green-500 rounded hover:bg-green-600">Mark
                                                as Completed</button>
                                        </form>
                                        <form
                                            action="{{ route('carpooling_details.cancelTrip', $carpoolingDetail->id) }}"
                                            method="POST" x-data>
                                            @csrf
                                            @method('PUT')
                                            <button
                                                @click.prevent="$dispatch('cancelTrip', { redirectUrl: 'carpooling_details', id: {{ $carpoolingDetail->id }}, modelClass: 'App\\Models\\CarpoolingDetails' })"
                                                type="submit"
